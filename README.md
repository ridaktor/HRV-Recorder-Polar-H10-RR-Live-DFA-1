...
- DFA scale range defaults to **4..12 beats** (closer to FatMaxxer `alpha1v2`).
- New flag **`--dfa_nmax`** (default **12**) to tweak the upper scale.
- **Experimental (quick mode):** fast but smoothed α1 via **`--alpha_fast_smooth`**  
  Uses a 15-s rolling **median** followed by a **time-aware EMA** (τ=20 s by default) for continuous feel.
